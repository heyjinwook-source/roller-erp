...
export default function PriceDbPage() {
  ...
  // ── 데이터 조회 ──────────────────────────────────────────
  async function fetchItems(name, spec, pageNum) {
    setLoading(true)
  ...
  async function loadPartNames() {
    ...
    while (true) {
      const { data } = await supabase
        .from('price_db').select('part_name').order('part_name')
        .range(from, from + 999)
  ...
  function goPage(p) { setPage(p); fetchItems(searchName, searchSpec, p) }
  ...
  function selectName(name) {
    setSearchName(name); setShowDropdown(false)
    setPage(1); fetchItems(name, searchSpec, 1)
  }
  ...
  async function handleDownload() {
    ...
    a.href = url; a.download = `단가DB_${new Date().toISOString().slice(0, 10)}.csv`
        ...
        <div className="px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {showDropdown && filteredNames.length > 0 && (
                <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
                  <div onClick={() => { setSearchName(''); setShowDropdown(false); setPage(1); fetchItems('', searchSpec, 1) }}
            ...
            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden focus-within:border-gray-400 bg-white">
              ...
              {searchSpec && (
                <button onClick={() => setSearchSpec('')} className="px-2 text-gray-300 hover:text-gray-500 text-lg leading-none">×</button>
              )}
            </div>

            {(searchName || searchSpec) && (
              <button onClick={() => { setSearchName(''); setSearchSpec(''); setPage(1); fetchItems('', '', 1) }}
                className="px-3 py-2 text-xs text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">
                초기화
              </button>
            )}
            <span className="text-xs text-gray-400 ml-auto">
              {loading ? '검색 중...' : `전체 ${total.toLocaleString()}건`}
            </span>
          </div>
        </div>
